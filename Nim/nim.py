...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        """
        rewards = []
        for possible_action in Nim.available_actions(state):
            key = (tuple(state), possible_action)
            rewards.append(self.q.get(key, 0))
    ...
    def choose_action(self, state, epsilon=True):
        ...
        # assume you are not going to perform a random choice
        fl_random_choice = False
        if epsilon:
            # it means we're training
            if random.random() < self.epsilon:
                fl_random_choice = True

        if fl_random_choice:
            return possible_actions.pop()

        # TODO: implement best action
        current_reward = -1e6
        candidate_action = None

        for action in possible_actions:
            key = (tuple(state), action)
            candidate_reward = self.q.get(key, 0)
